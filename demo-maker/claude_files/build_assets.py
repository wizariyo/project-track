...
a frame number — it all comes from here.

Pipeline:
  1. Slice ProjectTrack_Epic_Demo.mp4 into 6 physically separate 10s clips
     (fixes the startFrom() black-screen bug — Remotion never seeks inside
...
# Extra silent B-roll time appended AFTER each voiceover ends, purely visual —
...
def slice_clip(start_ts: str, duration_sec: int, out_path: Path) -> None:
    ...
    startFrom in Remotion — this IS the seek)."""
    cmd = [
        "ffmpeg", "-y",
        "-ss", start_ts,
        "-i", SOURCE_VIDEO,
...
async def build() -> None:
    ...
    if Path(MAC_FRAME_SRC).exists():
        ...
        print(f"✔ Copied {MAC_FRAME_SRC} -> {OUTPUT_DIR / 'mac_frame.png'}")
    else:
        print(f"⚠ Warning: {MAC_FRAME_SRC} not found next to this script. "
    ...
    for scene in SCENES:
        ...
        print(f"    → slicing @ {scene['start_ts']} for {CLIP_DURATION_SEC}s ...")
        slice_clip(scene["start_ts"], CLIP_DURATION_SEC, clip_path)

        print(f"    → generating voiceover: \"{scene['script']}\"")
        ...
        print(f"    → VO duration: {audio_ms}ms  |  scene length: {scene_duration}f "
    ...
    print(f"\n✅ Done. Total video length: {cursor_frame} frames "
...
if __name__ == "__main__":
    if not Path(SOURCE_VIDEO).exists():
        sys.exit(f"❌ Cannot find {SOURCE_VIDEO} — place it next to this script and re-run.")
    asyncio.run(build())
