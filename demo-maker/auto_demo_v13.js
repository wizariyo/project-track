...
(async () => {
  console.log("🚀 STARTING V13 E2E DEMO (100% BULLETPROOF)");
  ...
  const interactVisual = async (selector) => {
    try {
      ...
      let targetX = 960, targetY = 540; // center fallback
  ...
  // ============================================
  await page.goto('https://wizariyo.github.io/project-track/', { waitUntil: 'networkidle2' });
  await new Promise(r => setTimeout(r, 2500)); // Crucial wait for DOM to fully paint
  await injectCursor();
  
  await interactVisual('#roleStudent');
  await page.evaluate(() => { if(window.selectRole) window.selectRole('student'); }); // Guaranteed transition
  ...
  await page.evaluate((uid) => {
    ...
    // forcefully assign Alex UID to the dropdown
