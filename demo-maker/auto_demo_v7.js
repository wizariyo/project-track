...
(async () => {
  console.log("🚀 STARTING THE MASTERPIECE V7 E2E DEMO");
  ...
  await new Promise(r => setTimeout(r, 2000)); // wait for leads to load
  await page.evaluate(() => {
    const l = document.querySelector('#newGroupLead');
    if (l && l.options.length > 1) { l.selectedIndex = l.options.length - 1; l.dispatchEvent(new Event('change')); }
  });
  await new Promise(r => setTimeout(r, 500));
  ...
  await new Promise(r => setTimeout(r, 1500));
  // Teacher clicks Inspect Chat tab
  await page.evaluate(() => {
    const ct = document.querySelector('[data-tab="inspect-tab-chat"]');
    if(ct) ct.click();
  });
  await new Promise(r => setTimeout(r, 2000));
