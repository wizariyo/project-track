      ...
      try {
        ...
        if (!data || !data.length) {
          ...
          resultsEl.innerHTML = data.map(item => {
            let icon = '';
            if (item.type === 'Task') icon = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
            else if (item.type === 'Group') icon = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>';
            else icon = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>';
...
window.loadStudentChatChannels = async function() {
  ...
  channels.forEach(ch => {
    ...
    el.id = `chat-ch-${ch.id}`;
    let icon = '#';
    if(ch.type === 'faculty') icon = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"></path><path d="M6 12v5c3 3 9 3 12 0v-5"></path></svg>';
    if(ch.type === 'dm') icon = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>';
...
window.loadTeacherChatChannels = async function() {
  ...
  Object.keys(subjectsMap).forEach(subjName => {
    ...
    subjectsMap[subjName].forEach(g => {
      ...
      el.id = `chat-ch-${chId}`;
      el.innerHTML = `
        <div class="ch-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg></div>
        <div class="ch-details">
...
window.exportCombinedGradesReport = async function() {
  ...
  try {
    ...
    const activeSem = sessionStorage.getItem('activeTeacherSemester');
    if (activeSem) {
      groups = groups.filter(g => { let sem = g.semester; if (!sem && window.SUBJECT_CATALOG && g.subject) { const subj = window.SUBJECT_CATALOG.find(s => s.name === g.subject); if (subj) sem = subj.semester; } return String(sem) === String(activeSem); });
    }
  ...
  if (alreadyDone) {
    container.innerHTML = `
      <div class="card" style="padding:24px; text-align:center;">
        <div style="margin-bottom:8px;"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--teal)"><polyline points="20 6 9 17 4 12"></polyline></svg></div>
            ...
            return `
              <div style="text-align:center;">
                ...
                <div class="star-rating" data-peer="${pid}" data-cat="${cat}">
                  ${[1,2,3,4,5].map(s => `<span class="star ${s <= val ? 'active' : ''}" data-val="${s}" onclick="window.setStarRating('${pid}','${cat}',${s})" style="cursor:pointer; font-size:20px; color:${s <= val ? '#d4aa3a' : 'var(--border)'}; transition:color 0.15s;"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg></span>`).join('')}
                </div>
              </div>`;
          }).join('')}
        </div>
            ...
            ${Object.keys(scores).map(uid => {
              ...
              return `
                <tr style="border-bottom:1px solid var(--border);">
                  ...
                  <td style="text-align:center; padding:12px; font-weight:600;">${c1} <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg></td>
                  <td style="text-align:center; padding:12px; font-weight:600;">${c2} <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg></td>
                  <td style="text-align:center; padding:12px; font-weight:600;">${c3} <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg></td>
