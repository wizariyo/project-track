  ...
  document.getElementById('submitLogTimeBtn')?.addEventListener('click', async () => {
    ...
    try {
      await fetch(`/api/tasks/${taskId}/time`, {
        ...
        body: JSON.stringify({ addedTime: addedTimeSecs })
      });
  ...
  document.getElementById('submitEditProfileBtn')?.addEventListener('click', async () => {
    ...
    try {
      if (selfieBase64) {
        ...
        const r = await fetch(`/api/users/${user.id||user._id}/photo`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` },
          body: formData
        });
        if (!r.ok) {
          let errText = 'Failed to upload selfie.';
          try { const d = await r.json(); if (d.error) errText = d.error; } catch(e) {}
          throw new Error(errText);
        }
        const resData = await r.json();
        if (resData.success) {
          user.photoUrl = resData.photoUrl;
        } else {
          throw new Error(resData.error || 'Failed to upload selfie.');
        }
        ...
        if (!r.ok) {
          let errText = 'Failed to upload photo. It might be too large (Max 5MB).';
          try { const d = await r.json(); if (d.error) errText = d.error; } catch(e) {}
          throw new Error(errText);
        }
        const resData = await r.json();
    ...
    const drawFile = (f) => `
      <div style="background:var(--bg); border:1px solid var(--border); border-radius:var(--radius-sm); padding:12px; display:flex; flex-direction:column; gap:6px;">
        ...
        <div style="display:flex; gap:8px; margin-top:4px;">
          <a href="/api/files/download/${f.id}?token=${encodeURIComponent(getCurrentUser()?.token || '')}" class="btn btn-sm" style="flex:1; justify-content:center; background:var(--teal); color:var(--cream); padding:5px 8px; font-size:11px;">Download</a>
